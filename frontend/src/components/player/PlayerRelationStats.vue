<template>
  ...
  >
    <div v-if="mostFrequentPartners?.length" class="rounded-xl p-4 bg-gray-800">
      <div class="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Partenaires fréquents</div>
      <div
    ...
    <div v-if="bestPartners?.length" class="rounded-xl p-4 bg-gray-800">
      <div class="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Meilleurs partenaires</div>
      <div
      ...
      >
        ...
        <span class="text-xs text-green-400">{{ p.count > 0 ? Math.round((p.wins / p.count) * 100) : 0 }}% V</span>
      </div>
    </div>

    <div v-if="nemeses?.length" class="rounded-xl p-4 bg-gray-800">
      <div class="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Adversaires difficiles</div>
      <div
        v-for="p in nemeses"
        :key="p.playerId"
        class="flex justify-between items-center py-1.5 border-b border-gray-700 last:border-0"
