...
/**
 * A round's name as a translation key plus its interpolation values, resolved to
 * text only when the round is persisted. Naming rounds by key is what lets a
 * bracket generated by a French organizer read "Semi-finals" for an English viewer.
 */
...
/** Ordinal names of the last rounds, closest to the final first. */
const FINAL_ROUND_KEYS = [
  "FINAL",
  "SEMI_FINALS",
  "QUARTER_FINALS",
  "ROUND_OF_16",
...
/** Renders a round label in the language of the request that generated the bracket. */
function renderRoundName(label: RoundLabel): string {
  return t(`bracket.rounds.${label.key}`, label.params);
}
...
export class BracketService {
  ...
  private buildFirstRound(
    ...
    return {
      roundNumber: 0,
      label: this.getRoundLabel(0, totalRounds),
      bracketType: "winners",
  ...
  private buildLosersRounds(
    ...
    for (let i = 0; i < losersRoundsCount; i++) {
      ...
      rounds.push({
        ...
        label: { key: "LOSERS_ROUND", params: { round: i + 1 } },
        bracketType: "losers",
        matchesCount,
  ...
  /**
   * Get the round's label based on its position. The last five rounds have names
   * of their own; anything earlier is numbered.
   */
  private getRoundLabel(roundIndex: number, totalRounds: number): RoundLabel {
    const named = FINAL_ROUND_KEYS[totalRounds - roundIndex - 1];
    if (named) return { key: named };

    return { key: "ROUND", params: { round: roundIndex + 1 } };
  }
  ...
  private async createRoundsAndMatches(
    ...
    const createdRounds = await bracketRepository.createRounds(
      rounds.map((r) => ({
        ...
        // The rendered name is the fallback for clients that do not know the key.
        roundName: renderRoundName(r.label),
        roundNameKey: r.label.key,
        translationParams: r.label.params ?? null,
