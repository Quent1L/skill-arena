...
export class MatchService {
   ...
   */
  async confirmMatch(id: string, input: ConfirmMatchInput, confirmedBy: string) {
    const match = await this.getMatchById(id)
  ...
  async contestMatch(id: string, input: ContestMatchInput, contestedBy: string) {
    ...
    if (hasScoreProposal) {
      ...
      await this.notifyScoreProposal(id, contestedBy, input.proposedScoreA!, input.proposedScoreB!)
    } else {
   ...
   */
  private async notifyMatchCreated(matchId: string, createdBy: string, tournamentName: string) {
    const match = await matchRepository.getById(matchId)
    if (!match) return
    ...
    for (const playerId of playerIds) {
      ...
      const teammatesText = teammateNames.length > 0 ? teammateNames.join(', ') : 'Aucun'
   ...
   */
  private async notifyMatchValidationRequired(matchId: string, reportedBy: string) {
    const match = await matchRepository.getById(matchId)
    if (!match) return
